...
int Solver::threshold_factor(int round) {
  int factor = 1;
  while (true) {
    if (round & 1) {
      break;
...
void Solver::update_local(int j) {
  ...
  double value;
  double num = 0;
  for (Matrix::Entry** e = P.col(j)->first_entry(); e; e = P.col(j)->next_entry(e)) {
  ...
  double den = 0;
  for (Matrix::Entry** e = C.col(j)->first_entry(); e; e = C.col(j)->next_entry(e)) {
...
void Solver::increment(int j, int round) {
  ...
  local_for_phase = local;
        
  //    std::cout << "Local = " << local << std::endl;
        
  max = P.col(j)->max_entry();
  if (max) {
    threshold = (*max)->_value / factor;
    for (Matrix::Entry** e = P.first_entry_in_col(j); e; e = P.next_entry_in_col(j, e, threshold)){
      r = (*e)->_row;
...
void Solver::infeasible() {
  std::cout << " ---> Infeasible instance." << std::endl;
  terminate = true;
  feasible = false;
}

void Solver::print(double* arr, int len) {
  for (int i = 0; i < len; ++i) {
    std::cout << arr[i] << " ";
  }
  std::cout << std::endl;
...
void Solver::print(int* arr, int len) {
  for (int i = 0; i < len; ++i) {
    std::cout << arr[i] << " ";
  }
  std::cout << std::endl;
...
double Solver::abs_diff(double val1, double val2) {
