...
struct approx_sum_t {
  ...
  // Lower bound to sum is pow(1+epsilon, shift) * sum.
  // Upper bound to sum is pow(1+epsilon, shift) * (sum + # exponents).
  // Maintained so upper bound <= upper bound*(1+epsilon).
  ...
  void initialize(std::vector<exponent_t>& _exponents, double _epsilon);
  void initialize(std::vector<scalar_t>& values, double _epsilon);

  // epsilon in (-1,1) - {0}
  approx_sum_t(std::vector<exponent_t>& _exponents, double _epsilon) {
    initialize(_exponents, _epsilon);
  }

  approx_sum_t(size_t size, int exponent, double epsilon) {
    std::vector<exponent_t> _exponents(size, exponent);
    initialize(_exponents, epsilon);
  }

  approx_sum_t() : epsilon(0) {
  ...
  inline long double lower_bound() {
    return std::pow((long double)(1.0+epsilon), (long double) shift) 
      * (long double) sum; 
  }
