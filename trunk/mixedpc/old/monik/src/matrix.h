...
public:
	...
	inline Entry**	first_entry_in_row	(int row, double threshold = -1)  				{ if (_rows[row]) {return _rows[row]->first(threshold);} return NULL;}
	inline Entry**	next_entry_in_row	(int row, Entry** entry, double threshold = -1)	{ return _rows[row]->next(entry, threshold); }
	inline Entry**	first_entry_in_col	(int col, double threshold = -1)				{ if (_cols[col]) {return _cols[col]->first(threshold);} return NULL;}
	inline Entry**	next_entry_in_col	(int col, Entry** entry, double threshold = -1)	{ return _cols[col]->next(entry, threshold); }
	inline Entry**	max_entry_in_row	(int row)										{ return _rows[row]->first(); }
	inline Entry**	max_entry_in_col	(int col)										{ return _cols[col]->first(); }
	inline void		remove_col			(int col)										{ _cols[col]->remove_all(); --_n_cols_curr;}
	inline void		remove_row			(int row)										{ _rows[row]->remove_all(); --_n_rows_curr;}
	inline int		n_rows				()												{ return _n_rows; }
	inline int		n_cols				()												{ return _n_cols; }
	// current rows / cols keep track of active rows and columns
	inline int		n_rows_curr			()												{ return _n_rows_curr; }
	inline int		n_cols_curr			()												{ return _n_cols_curr; }
...
private:
	...
	struct EntryVector {
		...
		inline Entry** first(double threshold = -1) { return next(_lo-1, threshold); }
		inline Entry** next(Entry** entry, double threshold = -1) {
			while (++entry <= _hi)  {
				if (entry[0]->_removed) {
