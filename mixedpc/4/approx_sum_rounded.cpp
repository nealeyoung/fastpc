...
void
approx_sum_t::initialize(std::vector<scalar_t>& values, double _delta) {

  delta = _delta;

  assert (std::abs(delta) < 1.0  &&  delta != 0.0);
  std::vector<exponent_t> exponents(values.size()); // hides this->exponents
  for (size_t i = 0;  i < values.size();  ++i)
    exponents.at(i) = _scalar_to_exponent(values.at(i));

  initialize(exponents, _delta);
}

void
approx_sum_t::initialize(std::vector<exponent_t>& _exponents, double _delta) {
  ...
  shift = 0;
  tmp = std::floor(std::pow((long double) (1+delta), tmp));
...
approx_sum_t::raise_exponent(size_t k, exponent_t exponent) {
  ASSERT(delta != 0, "approx_sum_t::raise_exponent called on uninitialized approx_sum");
  ...
  if (delta > 0) {
    ...
    if (sum < exponents.size() / std::abs(delta))
      _reset_shift();
  }

  // assert error bound
  assert (sum >= exponents.size() / std::abs(delta));

  // for debugging
  ASSERT(upper_bound() <= (1+std::abs(delta))*lower_bound(),
         _(sum)
         _(delta)
         _(exponents.size())
         _(lower_bound())
...
approx_sum_t::_scalar_to_exponent(scalar_t scalar) {
  ...
  long double tmp = std::log((long double) scalar) / std::log1p(delta);
...
approx_sum_t::_exponent_to_rounded(exponent_t exponent) {
  ...
  long double tmp = std::floor(std::pow((long double) (1+delta), (long double)(shifted)));
...
approx_sum_t::_reset_shift() {
  ...
  if (delta > 0) {
    ...
    long double tmp = std::ceil(std::log(std::ceil((long double) exponents.size()/delta))
                                / std::log1p((long double) delta));
    ...
    assert (sum >= exponents.size()/delta);
    sanity_check();
...
approx_sum_t::sanity_check() {
  ...
  // for debugging
  assert (upper_bound() <= (1+std::abs(delta))*upper_bound());
  ...
  for (auto e : exponents) {
    ...
    auto power = std::pow((long double)(1.0+delta), (long double)(e - shift));
  ...
  ASSERT(err1 <= std::abs(delta), // && err2 <= std::abs(delta),
         _(lb)
         _(ub - lb)
         _(delta)
         _(err1)
...
scalar_t approx_sum_t::times(approx_sum_t & other) {
  assert (other.delta * delta < 0);
  // (1+other.delta)**other.shift * other.sum
  //   * (1+delta)**shift * sum
  return
    ((std::pow((1+other.delta)*(1+delta), shift)
      * std::pow(1+other.delta, other.shift-shift))
     * scalar_t(sum))
    * scalar_t(other.sum);
}
scalar_t approx_sum_t::divided_by(approx_sum_t & other) {
  ...
  //   / (1+other.delta)**other.shift * other.sum
  return
    (std::pow((1+delta)/(1+other.delta), shift)
     / std::pow(1+other.delta, other.shift - shift))
