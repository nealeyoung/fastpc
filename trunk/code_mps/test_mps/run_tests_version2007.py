        ...
        for fp_file in fastpc_files:
            if fp_file.startswith(input_file_prefix) and not fp_file.startswith('.'):
                ...
                for eps in epsilons:
                    ...
                    cmd_exact = '../../version_2007/code/fastpc' + ' ' + str(eps) + ' < ./test_cases/' + fp_file + ' 1 >> ' + output_file_location + '_version_2007'
                    os.system(cmd_exact)
                    if (a_sort_ratio > 1):
                        #run with sort_ratio > 1 (approx sorting)
                        cmd_approx = '../../version_2007/code/fastpc' + ' ' + str(eps) + ' < ./test_cases/' + fp_file + ' ' + str(a_sort_ratio) + ' >> ' + output_file_location + '_version_2007'
