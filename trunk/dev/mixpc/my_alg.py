...
while True:
    ...
    #do all incraments for one round robin 
    all_deleted , x = find_incrament(prob,x) 
    #all_deleted = inc_Px_Cx(j,inc_amount,prob)
    #x[j] = x[j] + inc_amount
...
print prob.adds
print '(m+n)*m*log(m)/eps^2 number of checks of ratio in worst case'
print ( (prob.m + prob.num_vars)*prob.m  * math.log(prob.m))/prob.eps**2
print 'actual checks'
print prob.checks
