...
# dimensions of atomic region in pixels **should evenly divide pixel width and height**
box_dim = 1  #keep at 1 to process image as-is (no blurring)
...
print "Processing",width,"x",height

data= list(converted.getdata())
array = []  # stores bitmap as 2d array row by row; [0][0] is top-left corner
for i in range(0,len(data),width):
    array.append(data[i:i+width])

# generate relaxed image, i.e. lower resolution-- to allow fewer vars and constraints 
relaxed_data = []
for i in range(0,len(array),box_dim):
    for j in range(0,len(array[0]),box_dim):
        relaxed_data.append(sum([array[y][x] for y in range(i, i+box_dim) for x in range(j, j+box_dim)])/(box_dim*box_dim))

relaxed_array = []  # 2d array of relaxed image
for i in range(0,len(relaxed_data),width/box_dim):
    relaxed_array.append(relaxed_data[i:i+(width/box_dim)])

# debug-- view relaxedned image
#relaxed_image = Image.new("L",(width/box_dim, height/box_dim))
#relaxed_image.putdata(relaxed_data)
#relaxed_image.show()

#reset vars for relaxed array
width = width/box_dim
height = height/box_dim
data = relaxed_data
array = relaxed_array
