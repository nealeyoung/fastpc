...
def find_incrament(prob,x):
    ...
    for j in range(prob.num_vars):
        if prob.c_var_i[j].val is None  or prob.p_var_i[j].val is None:
            continue
        ...
        if len(prob.c_cons_i) < 1:
            print 'tried to update a varibable not in any covering constraint'
            valid.append( (j,0))
...
def ratio(j,prob):
    ...
    eCx = map(lambda x: math.e**x if x != "deleted" else "deleted",prob.Cx)
    ePx = map(lambda x: math.e**x if x != "deleted" else "deleted",prob.Px)

    MijeCx = [coeff.val*eCx[coeff.cons] for coeff in prob.c_var_i[j].generate()]
...
def drop_cons(i,prob):
    ...
    prob.Cx[i] = 'deleted' #check this should not be needed after fix below
    for coeff in prob.c_cons_i[i]:
        coeff.node.remove()
    prob.c_cons_i[i] = "deleted" 

#also need to update overalls for ratios
